...
export function GroupSelectionModal({ song, groups, isOpen, onClose, onAddToGroups }: GroupSelectionModalProps) {
  ...
  const handleAddToGroups = async () => {
    if (!song || selectedGroupIds.length === 0 || isSubmitting) return;

    setIsSubmitting(true);
    setError(null);

    try {
      await onAddToGroups(song.id, selectedGroupIds);
      setSelectedGroupIds([]);
      onClose();
    } catch (err) {
      console.error(err);
      setError(err instanceof Error ? err.message : '그룹에 곡을 추가하지 못했습니다.');
    } finally {
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the groups where you want to add &quot;{song.title}&quot; by {song.artistName}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button
              ...
              disabled={selectedGroupIds.length === 0 || groups.length === 0 || isSubmitting}
            >
